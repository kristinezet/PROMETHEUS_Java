...
public class Tradepoints implements Serializable{
	...
	File f;
	ArrayList<Tradepoint> tradepoints;

	public Tradepoints(String filename){
		this.tradepoints = new ArrayList<>();
	...
	private void read(){
		...
		try {
			...
			tradepoints = (ArrayList<Tradepoint>)dec.readObject();
	...
	public void add(Tradepoint tr) {
		tradepoints.add(tr);
		save();
	}

	public void remove(Tradepoint t) {
		tradepoints.remove(t);
		save();
	}
}
